...
/**
 * 페이징 처리위한 bean <br>
 * page : 데이터(content)들의 묶음
 * page group : page 묶음
 * @author kgmyh
 *
 */
public class TpNoticeBoardPagingBean {
	/**
	 * 총 데이터(내용들:rows)의 개수
	 */
	 ...
	 */
	public static int PAGE_PER_PAGEGROUP = 10;
	 ...
	 * @param currentPage
	 */
	public TpNoticeBoardPagingBean(int totalContent, int currentPage){
	...
	private int getTotalPage(){
		int totalPage = 0;
		if(totalContent%CONTENT_PER_PAGE != 0){
			totalPage = totalContent/CONTENT_PER_PAGE +1;
		}else{
			totalPage = totalContent/CONTENT_PER_PAGE;
		}		
		return totalPage;
	}
	/**
	...
	private int getTotalPageGroup(){
		...
		if(totalPage%PAGE_PER_PAGEGROUP != 0){
			totalPageGoup = totalPage/PAGE_PER_PAGEGROUP + 1;
		}else{
			totalPageGoup = totalPage/PAGE_PER_PAGEGROUP;
		}		
	...
	private int getCurrentPageGroup(){
		...
		if(currentPage%PAGE_PER_PAGEGROUP != 0){
			currentPageGroup = currentPage / PAGE_PER_PAGEGROUP +1;
		}else{
			currentPageGroup = currentPage / PAGE_PER_PAGEGROUP;
		}
	...
	public int getStartPageOfPageGroup(){
		int startPageNo = (getCurrentPageGroup()-1)*PAGE_PER_PAGEGROUP+1;
		return startPageNo;
	}
	/**
	...
	public int getEndPageOfPageGroup(){
		int endPageNo =getCurrentPageGroup()*PAGE_PER_PAGEGROUP;
	...
	public static void main(String[] args) {
		TpNoticeBoardPagingBean d = new TpNoticeBoardPagingBean(201, 5);//총 201개 row, 현재page : 5
		//page내 보여질 내용의 개수 - 5
		//page그룹내 page수 - 10
