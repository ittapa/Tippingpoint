...
public class TpProjectValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		// 시작일과 마감일 관련 처리
		// 미입력
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "tppFundingStartDate", "selected", new Object[] { "시작일" },
				"필수입력사항입니다.");
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "tppFundingLastDate", "selected", new Object[] { "마감일" },
				"필수입력사항입니다.");
		...
		// TODO :
		System.out.println(tpProject.getTppState() + "프로젝트 요청상태");
		if (tpProject.getTppState().equals("A") && (tpProject.getTppFundingStartDate().length() != 8
				|| Integer.parseInt(tpProject.getTppFundingStartDate()) < numToday)) {
			System.out.println("시작날짜 검증실패");
			errors.rejectValue("tppFundingStartDate", "mistake", "유효하지 않은 날자 입니다.");
		}

		if (tpProject.getTppFundingLastDate().length() != 8
				|| Integer.parseInt(tpProject.getTppFundingLastDate()) < numToday) {
			errors.rejectValue("tppFundingLastDate", "mistake", "유효하지 않은 날자 입니다.");
		}

		if (Integer.parseInt(tpProject.getTppFundingLastDate()) < Integer
				.parseInt(tpProject.getTppFundingStartDate())) {
			errors.rejectValue("tppFundingLastDate", "mistake", "마감일이 시작일보다 빠릅니다.");

		}
		System.out.println("오늘" + numToday + " 시작일" + tpProject.getTppFundingStartDate() + " 마감일 "
				+ tpProject.getTppFundingLastDate());

		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "tppTargetAmount", "required", new Object[] { "목표금액" },
				"목표 금액을 입력해주세요");
		...
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "tppProjectContent", "null", new Object[] { "내용" },
				"내용을 입력하세요");
